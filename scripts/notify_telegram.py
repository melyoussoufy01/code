...
def calc(x):
    ...
    neutral_price_prudent=max(0,(cash_before_finance-reserve_month)/factor) if factor else 0
    neutral_price_current=max(0,cash_before_finance/factor) if factor else 0
    return {'payment':payment,'effort_current':effort_current,'effort_prudent':effort_prudent,'gross':gross,'neutral_price_prudent':neutral_price_prudent,'neutral_price_current':neutral_price_current}
...
for x in current.get('listings',[]):
    ...
    # Strict Houser rule: primary signal is prudent all-in effort.
    actionable=metrics['effort_prudent']<=100
    meaningful_drop=drop>=0.03 and metrics['effort_prudent']<=120
    explicit=bool(x.get('telegramNotify'))
    if not ((is_new and actionable) or meaningful_drop or explicit):
        continue

    if metrics['effort_prudent']<=0:
        quality='Pépite'
    elif metrics['effort_prudent']<=50:
        quality='Très solide'
    else:
        quality='À creuser seulement si actif supérieur'

    reason='Nouveau match' if is_new else (f'Baisse de prix -{drop*100:.1f} %' if drop else 'Match Houser')
    alerts.append((metrics['effort_prudent'],f"🏠 HOUSER — {reason}\n{quality}\n{x.get('city','')} · {x.get('area','')}\nPrix : {euro(price)}\nLoyer prudent : {euro(float(x.get('rentEstimateHC') or 0))} HC/mois\nMensualité Mourabaha basse : {euro(metrics['payment'])}/mois\nEffort prudent tout compris : {'-' if metrics['effort_prudent']<0 else '+'}{euro(abs(metrics['effort_prudent']))}/mois\nEffort mois normal : {'-' if metrics['effort_current']<0 else '+'}{euro(abs(metrics['effort_current']))}/mois\nRendement brut : {metrics['gross']*100:.1f} %\nPrix max ~0 € effort prudent : ~{euro(metrics['neutral_price_prudent'])}\nAnnonce : {x.get('url','')}\nHouser : {HOUSER_URL}"))
